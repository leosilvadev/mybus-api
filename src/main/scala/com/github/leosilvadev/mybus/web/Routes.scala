...
import io.vertx.scala.ext.web.handler.{ErrorHandler, LoggerHandler, TimeoutHandler}

object Routes {

  def all(vertx: Vertx, serverConfig: ServerConfig, lines: List[Line], stopTimes: List[StopTime], delays: List[Delay]): Router = {
